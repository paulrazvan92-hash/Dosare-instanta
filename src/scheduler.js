...
async function checkDosar(dosar) {
  try {
    ...
    if (!fresh) {
      await db.updateDosarSnapshot(dosar.id, null, 'Dosarul nu a fost gasit pe portal.just.ro');
      return { ok: false, events: [] };
    }

    const events = compareDosare(dosar.snapshot, fresh);
    await db.updateDosarSnapshot(dosar.id, fresh, null);

    const label = dosar.label || dosar.numarDosar;
    for (const ev of events) {
      await db.addEvent(dosar.id, ev.type, ev.message);
    ...
    await db.updateDosarSnapshot(dosar.id, null, err.message);
    return { ok: false, events: [], error: err.message };
  }
}

async function checkAll() {
  const dosare = await db.listDosare();
